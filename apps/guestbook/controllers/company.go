...
func NewCompanyController(helper *helper.Helper, service services.Company) *CompanyController {
...
func (s *CompanyController) Add(c *fiber.Ctx) error {
	tracerCtx := c.UserContext()

	_, span := s.helper.Utils.JaegerTracer.StartSpan(tracerCtx, "guestbook.Company_controllers", "add")
	defer span.End()
...
func (s *CompanyController) GetAll(c *fiber.Ctx) error {
	tracerCtx := c.UserContext()
...
func (s *CompanyController) Delete(c *fiber.Ctx) error {
	tracerCtx := c.UserContext()

	_, span := s.helper.Utils.JaegerTracer.StartSpan(tracerCtx, "guestbook.Company_controllers", "delete")
	defer span.End()
